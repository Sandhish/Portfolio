        ...
        {PROJECTS.map((project, index) => (
          <div key={index} className='mb-4 lg:w-3/4 flex flex-wrap justify-center border border-neutral-800 rounded-3xl'>
            <motion.div whileInView={{ opacity: 1, x: 0 }} initial={{ opacity: 0, x: -100 }} transition={{ duration: 1 }} className='w-full lg:w-1/4 flex justify-center'>
              <img src={project.image} className='my-3 rounded-2xl p-2' alt={project.title} style={{ height: '220px' }} />
            </motion.div>
            ...
            <motion.div whileInView={{ opacity: 1, x: 0 }} initial={{ opacity: 0, x: 100 }} transition={{ duration: 1 }} className="w-full max-w-xl lg:w-full p-5">
              ...
              <div className='flex justify-center lg:justify-start lg:ml-4 mt-3'>
                <motion.a href={project.demo} whileHover={iconHover} target='_blank' rel='noopener noreferrer' className='mr-4 cursor-pointer bg-purple-800 text-sm text-white px-3 rounded-3xl flex items-center'>
                  Try Now <span className='ml-2'>&rarr;</span>
                </motion.a>

                <motion.a href={project.github} whileHover={iconHover} target='_blank' rel='noopener noreferrer' className='bg-neutral-900 text-sm text-white px-4 py-2 rounded-lg'>
                  GitHub
                </motion.a>
              </div>
